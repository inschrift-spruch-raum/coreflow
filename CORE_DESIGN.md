...
并发实现最大的 Rust 细节是 ownership。当 plug 存在 `BTreeMap<String, PlugEntry>` 中，并且 plug 是 Rust serde closure 时，多个 ticked plug spawn 到 `JoinSet` 需要可拥有的 plug handle、内部锁或可 clone executor。反馈 flow 还意味着同一个 plug 可能在同一次 run 中收到多次 tick，因此实现必须决定同一 plug 的重复 tick 是串行、可并发，还是由 Plug 的 `reentrant` 决定。

第一种是让 plug entry 存为 `Arc<Mutex<PlugEntry>>`。spawn 时 clone Arc，future 内 lock plug 并调用。优点是实现简单，可支持 stateful `FnMut`，并自然串行化同一 plug 的重复 tick。缺点是每个 plug 调用有锁开销；如果某个 plug 标记为 `reentrant` 且希望同一 plug 多 tick 并发，这个方案会过于保守。
...
第三种是要求 `F: Fn(I) -> Fut + Clone + Send + Sync`，每次 tick clone plug。优点是并发自然，适合 stateless reentrant plug；缺点是放弃 `FnMut` 能力，也限制 stateful plug。

本标准采用 `Arc<Mutex<_>>` 或 `Arc<tokio::sync::Mutex<_>>` 作为最小可行方案，同时在 Plug 中记录 `reentrant`。默认语义是同一 plug 的多个 tick 串行，不同 plug 之间并发。cloneable stateless plug 优化服务于同一 plug 的多个 tick 并发复用。
...
如果 Plug 声明带资源标签，例如 `resource: "openai"`、`resource: "filesystem"`、`resource: "gpu"`，scheduler policy 可以通过 `ExecutionPolicy.resource_limits` 限制每类资源的并发数。资源标签属于 Plug 调度声明；ExecutionPolicy 只读取这些标签并决定调度，不拥有业务或资源事实。资源限制只阻塞同资源且达到上限的 tick，不应阻塞无关资源的 ready work。
