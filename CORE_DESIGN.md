...
`Graph` 构建环节可以收集 GraphChange；`commit(message)` 环节把当前暂存的 GraphChange 写入新的 GraphCommit；`check` 环节检查未知 plug、flow path、Plug 和启动条件，并刷新执行缓存；`run` 环节必须自动执行 check 并消费 checked Graph。反馈环不在 check 环节被拒绝；run 环节只用 tick、change detection 和 Idle 处理传播，并产出 GraphResult。
...
用户希望 workflow 具备自组织、自调整、自适应能力。kernel 执行当前 checked Graph；负责修改 graph 的普通 plug 读取 Graph、GraphResult、资源状态和用户目标，产出 `GraphChange` 或新的 Graph，Graph 应用 change 后通过 `commit(message)` 写入新的 GraphCommit。

这种 plug 仍是普通 plug：输入是可序列化的 graph 事实和运行事实，输出是可序列化的 `GraphChange` 或 next Graph。Graph 负责应用修改、check 和 commit。

自适应模型有三步。第一，宿主把当前 GraphStore、上一次 GraphResult、资源状态和用户目标编码进普通 initial input、seed input 或上游 plug 输出；coreflow 不隐式注入这些事实。第二，普通 plug 根据这些可序列化事实生成 `GraphChange` 或 next Graph。第三，可变 Graph 运行入口应用 graph change，执行 check，并写入新的 GraphCommit；kernel 执行新的 checked Graph。这样系统可以自适应，但每一次执行都绑定一个明确 GraphCommit id。
...
默认可变 `run` 入口用输出 shape 识别自适应修改：一次 run 到达 Idle 后，Graph 遍历 plug 输出；如果某个输出可以反序列化为 `GraphChange`，就应用、暂存并通过 `commit(...)` 提交；如果某个输出可以反序列化为完整 `Graph`，就按 `GraphChange::Replace` 替换当前 Graph 并提交。识别出的自适应输出不会作为下一轮 initial input 继续传播，以避免同一个 graph change 重复执行。业务 plug 如果可能输出与 `GraphChange` 或 `Graph` 同形的 JSON，应通过 schema、plug kind 或宿主约定避免误触发。
...
Graph 运行入口保持单一：`run` 是默认可变运行入口，会把运行中产生的 `GraphChange` 通过 `commit(...)` 持久化回调用者 Graph。默认调用写作 `graph.run(json!({ ... })).await?`；带配置调用写作 `graph.run(Run::new(json!({ ... })).policy(policy).picker(picker).seeds(["adapt"])).await?`。`Run` 是一次运行请求，不是新的执行器。
...
自适应 graph 修改由普通 Rust plug 产出 `GraphChange` 或 next Graph。宿主通过普通输入提供当前 GraphStore、上一次 GraphResult、资源状态和用户目标等事实；coreflow 只要求这些事实可序列化。默认 `run` 入口应用修改后执行 check 并写入新的 GraphCommit。
...
Graph 存储叙事围绕单一 `graph.json`、当前完整 Graph 和 GraphCommit 更改记录展开。PlugInput 构建叙事围绕 declarative selector、InputBind 和 serde 反序列化展开。自适应叙事围绕普通 Rust plug 产出的 `GraphChange` 或 next Graph 展开。
